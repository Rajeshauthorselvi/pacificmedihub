  ...
  <td colspan="4" style="padding:0">
    <table class="table table-bordered" style="width: 100%">
      <tr class="accordion-toggle collapsed" id="accordion{{ $product_id }}" data-toggle="collapse" data-parent="#accordion{{ $product_id }}" href="#collapse{{ $product_id }}">
        ...
        {{-- <td colspan="2">Quantity:&nbsp;<span class="total_quantity">0.00</span></td>
        <td class="total-head text-center" colspan="2">Total:&nbsp;<span class="total">0.00</span></td> --}}
        <td style="width: 5%;">
      ...
      <tbody id="collapse{{ $product_id }}" class="collapse in p-3">
          ...
          <th class="width">Discount
            <div class="discount-type">
              <div class="icheck-info d-inline">
                <input type="radio" name="new_variant[discount_type][{{ $product_id }}]" checked id="percentage{{ $product_id }}" class="dis-type" value="percentage"><label for="percentage{{ $product_id }}">%</label>
              </div>&nbsp;&nbsp;&nbsp;
              <div class="icheck-info d-inline">
                <input type="radio" class="dis-type" name="new_variant[discount_type][{{ $product_id }}]" id="amount{{ $product_id }}" value="amount"><label for="amount{{ $product_id }}">$</label>
              </div>
            </div>
          </th>
        ...
        @foreach($product_variant as $key=>$variant)
          <tr class="parent_tr">
            ...
            <input type="hidden" name="new_variant[id][]" value="{{$variant['variant_id']}}" class="variant_id">
            <td>
            ...
            <td>
              <input type="hidden" name="new_variant[base_price][]" value="{{$variant['base_price']}}">
              <input type="hidden" name="new_variant[retail_price][]" value="{{$variant['retail_price']}}">
              {{$variant['retail_price']}}
            </td>
            <td>
              <input type="hidden" name="new_variant[minimum_selling_price][]" class="minimum-price" value="{{$variant['minimum_selling_price']}}">
