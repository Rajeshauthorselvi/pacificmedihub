...
<div class="breadcrumbs">
	<div class="container">
		<ul class="items">
			...
			<li><a href="{{ route('cart.index') }}" title="My Cart Page">My Cart</a></li>
      <li><a>Request RFQ</a></li>
		</ul>
	</div>
</div>
		...
		<div class="row request-rfq">
			<div class="col-sm-6">
		      ...
		      <div class="address-block">
         	  <input type="hidden" name="delivery_address" id="deliveryAddID" value="{{ $delivery->id }}">
           	<h6>{{ $delivery->name }}</h6>
          ...
          <div class="icheck-info d-inline">
            ...
            <label for="sameAs">Billing address same as Delivery address</label>
          </div>

          <input type="hidden" name="billing_address" id="billingAddID" value="@if($delivery!=null){{ $delivery->id }}@else{{ $primary->id }}@endif">
          <div class="billing-address">
           	<h5>Billing Address</h5>
           	<select class="form-control select2bs4" id="billingAddress">
           		<option value="" selected>Please Select Address</option>
          ...
          <div class="rfq customer-notes">
            <div class="form-group">
              <h5>Notes</h5>
              <textarea class="form-control" rows="3" name="notes" id="rfqNotes">{{old('notes')}}</textarea>
            </div>
          </div>
			...
			<div class="col-sm-6">
			 ...
			 <button type="submit" class="btn save-btn send-rfq">Send RFQ</a>
		</div>
      </form>
	</div>
</div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="address-details">
            ...
            <input type="hidden" name="address[address_type]" id="addressType">
            ...
            <div class="form-group">
            	<div class="col-sm-6">
              	<button type="button" class="btn reset-btn close-modal" data-dismiss="modal">Cancel</button>
...
@push('custom-scripts')
  ...
  <script type="text/javascript">
    ...
    $('#deliveryAddress').on('change',function() {
      var delID = $("option:selected", this).val();
      if(delID=='new'){
    ...
    $('#cancelChangeBilling').on('click',function () {
      $('#sameAs').prop('checked', true);
      $('.billing-address').hide();
      var billID = $('#deliveryAddID').val();
      $('#billingAddID').val(billID);
    ...
    $(document).ready(function($) {
      $('.close-modal').on('click',function(){
        ...
        $('#deliveryAddress').val(null).trigger("change")
      });
    });
    ...
    $('.send-rfq').on('click',function(){
      if (!confirm('Are you sure to send a RFQ Request?')) return false;
      var delAddress  = $('#deliveryAddID').val();
      var billAddress = $('#billingAddID').val();
      if(delAddress==''){
        alert('Please check and confirm Delivery address.!');
        return false;
      }else if(billAddress==''){
        alert('Please check and confirm Billing address.!');
        return false;
      }else{
        $('#rfqForm').submit();
        return true;
